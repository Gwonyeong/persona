...
export default function GalleryUnlockModal({ image, characterId, onClose, onUnlocked }) {
  ...
  const handleUnlock = async () => {
    ...
    setError('')
    try {
      const result = await api.post(`/characters/${characterId}/gallery/${image.id}/unlock`)
      if (result.error) {
      ...
      <div className="relative bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-sm">
        ...
        <div className="text-center mb-4">
          <p className="text-lg font-bold text-gray-100">
            {image.missionName || image.title || '갤러리 이미지'}
          </p>
