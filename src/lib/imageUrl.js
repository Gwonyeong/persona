...
export const IMG_W = {
  ...
  // 2열 카드. 컨테이너는 218css 폭 x 327css 높이(aspect-2/3)인데, 기준은 폭이 아니라
  // "덮는 쪽" 치수여야 한다. object-cover는 모자란 축에 맞춰 확대하므로 정사각 원본은
  // 세로에 맞춰 늘어나 렌더 폭이 327css가 된다(폭 218 기준으로 받으면 2배 업스케일).
  // 실제로 이소민(#55)은 profileImage가 세로 프로필이 아니라 정사각 960x960 스타일
  // 표정 이미지라 이 경로로 깨져 보였다. 원본보다 큰 폭 요청은 원본 해상도로 잘리므로
  // 세로 원본에서 낭비도 없다.
  CARD: 336,
  HERO: 448, // 전폭 슬라이더
}
