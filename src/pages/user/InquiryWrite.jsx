...
export default function InquiryWrite() {
  ...
  const handleSubmit = async () => {
    if (!title.trim()) { setError(t('inquiry.titleRequired')); return }
    if (!content.trim()) { setError(t('inquiry.contentRequired')); return }
  ...
  return (
    <div className="px-4 pt-4 pb-4">
      <Helmet>
        <title>{t('inquiry.writeTitle')} - Pesona</title>
      </Helmet>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">{t('inquiry.writeTitle')}</h1>
        </div>
        <button
      ...
      <div className="mb-4 px-3 py-2.5 bg-indigo-600/10 border border-indigo-500/20 rounded-xl">
        <p className="text-xs text-indigo-300 text-center">{t('inquiry.notice')}</p>
      </div>
